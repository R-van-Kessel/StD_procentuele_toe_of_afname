...
function generateQuestions() {
    ...
    // ===== MAAK 10 PROCENT VRAGEN =====
    for (let i = 0; i < 10; i++) {
        // Kies random vraagtype
        let questionType = floor(random(6));  // 6 verschillende types
        
        let text = "";
        let answer = 0;
        
        if (questionType === 0) {
            // TYPE 0: TOENAME - Gegeven oud + percentage, zoek nieuw
            // Formule: nieuw = oud × (1 + percentage/100)
            // Voorbeeld: €50 met 20% toename → €50 × 1.20 = €60
            
            let oud = floor(random(20, 200));           // Oude waarde (20-200)
            let percentage = floor(random(5, 50));      // Toename percentage (5-50%)
            let nieuw = oud * (1 + percentage / 100);   // Bereken nieuwe waarde
            
            text = "€" + oud + " + " + percentage + "% = ?";
            answer = round(nieuw * 100) / 100;  // 2 decimalen
            
        } else if (questionType === 1) {
            // TYPE 1: AFNAME - Gegeven oud + percentage, zoek nieuw
            // Formule: nieuw = oud × (1 - percentage/100)
            // Voorbeeld: €80 met 25% afname → €80 × 0.75 = €60
            
            let oud = floor(random(30, 200));
            let percentage = floor(random(10, 60));     // Afname percentage (10-60%)
            let nieuw = oud * (1 - percentage / 100);
            
            text = "€" + oud + " - " + percentage + "% = ?";
            answer = round(nieuw * 100) / 100;
            
        } else if (questionType === 2) {
            // TYPE 2: TOENAME - Gegeven nieuw + percentage, zoek oud
            // Formule: oud = nieuw / (1 + percentage/100)
            // Voorbeeld: Na 20% toename is het €60, wat was oud? → €60 / 1.20 = €50
            
            let percentage = floor(random(10, 40));
            let nieuw = floor(random(50, 250));
            let oud = nieuw / (1 + percentage / 100);
            
            text = "? + " + percentage + "% = €" + nieuw;
            answer = round(oud * 100) / 100;
            
        } else if (questionType === 3) {
            // TYPE 3: AFNAME - Gegeven nieuw + percentage, zoek oud
            // Formule: oud = nieuw / (1 - percentage/100)
            // Voorbeeld: Na 25% afname is het €60, wat was oud? → €60 / 0.75 = €80
            
            let percentage = floor(random(10, 50));
            let nieuw = floor(random(30, 200));
            let oud = nieuw / (1 - percentage / 100);
            ...
            // TYPE 4: TOENAME - Gegeven oud + nieuw, zoek percentage toename
            // Formule: percentage = ((nieuw - oud) / oud) × 100
            // Voorbeeld: €50 → €60, hoeveel % toename? → ((60-50)/50) × 100 = 20%
            
            let oud = floor(random(20, 150));
            let toename = floor(random(10, 60));         // Absolute toename
            let nieuw = oud + toename;
            let percentage = ((nieuw - oud) / oud) * 100;
            
            text = "€" + oud + " → €" + nieuw + " = ?%";
            answer = round(percentage * 10) / 10;  // 1 decimaal (bijv. 25.5%)
            
        } else {
            // TYPE 5: AFNAME - Gegeven oud + nieuw, zoek percentage afname
            // Formule: percentage = ((oud - nieuw) / oud) × 100
            // Voorbeeld: €80 → €60, hoeveel % afname? → ((80-60)/80) × 100 = 25%
            
            let nieuw = floor(random(30, 150));
            let afname = floor(random(10, 60));
            let oud = nieuw + afname;
            let percentage = ((oud - nieuw) / oud) * 100;
            
            text = "€" + oud + " → €" + nieuw + " = ?%";
            answer = round(percentage * 10) / 10;  // 1 decimaal
        }
        
        // Check geldigheid (geen negatieve waardes, redelijke getallen)
        ...
        questions.push({ 
            text: text,
            answer: answer,
            triangleData: null  // Geen driehoek bij procent sommen
        });
        answers.push(answer);
    }
...
for (let row = 2; row < 4; row++) {
    for (let col = 0; col < COLS; col++) {
        ...
        // Bepaal of antwoord een bedrag (€) of percentage (%) is
        let vraagTekst = questions[answerIndex % 10].text;
        
        // Verander punt naar komma
        let answerMetKomma = answers[answerIndex].toString().replace('.', ',');
        
        if (vraagTekst.includes("?%")) {
            // Vraag naar percentage → antwoord is percentage
            answerText = answerMetKomma + "%";
        } else {
            // Vraag naar bedrag → antwoord is bedrag
            answerText = "€" + answerMetKomma;
        }
...
function drawBlock(block) {
    // Bepaal de kleur van het blokje
    if (isChecked && block.isCorrect !== null) {
        if (block.isCorrect) {
            fill(100, 200, 100);  // Groen
        } else {
            fill(250, 100, 100);  // Rood
        }
    } else if (block.isQuestion) {
        fill(100, 150, 250);      // Blauw voor vragen
    } else {
        fill(255, 200, 100);      // Oranje voor antwoorden
    }
    ...
    // Vaste tekstgrootte
    textSize(12);  // ← Aanpasbaar: verander dit getal
    
    // ALLEEN vraag blokjes met haakjes splitsen op 2 regels
    if (block.isQuestion && txt.length > 8 && txt.includes(" = ")) {
//                                           ← Voeg toe: alleen splitsen als langer dan 15 tekens
        // Dit is een vraag met haakjes → split bij "="
        let parts = txt.split(" = ");
        
        if (parts.length === 2) {
            let line1 = parts[0];      // Links van =
            let line2 = "= " + parts[1]; // Rechts van =
            
            // Teken op 2 regels
            text(line1, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2 - 8);
            text(line2, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2 + 8);
        } else {
            // Kan niet splitsen
            text(txt, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
        }
    } else {
        // Antwoord blokje of korte vraag → 1 regel
        text(txt, block.x + CELL_SIZE / 2, block.y + CELL_SIZE / 2);
    }
}
...
function drawBlock(block) {
    ...
    // Teken tekst (ALLEEN procentsommen)
    fill(255);
    noStroke();
    textAlign(CENTER, CENTER);
    textStyle(BOLD);
    
    // Pas tekstgrootte aan op basis van lengte
    if (block.text.length > 15) {
        textSize(10);  // ← AANPASBAAR: kleine tekst voor lange vragen
    } else if (block.text.length > 10) {
        textSize(11);  // ← AANPASBAAR: middelgrote tekst
    } else {
        textSize(13);  // ← AANPASBAAR: normale tekst
    }
