...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        let text = "";
        let answer = 0;
        let answerId = i;  // UNIEKE ID PER VRAAG
        
        if (questionType === 0) {
            // TYPE 0: TOENAME - Gegeven oud + percentage, zoek nieuw
            let oud = floor(random(20, 200));
            let percentage = floor(random(5, 50));
            ...
            let percentage = floor(random(10, 60));
            let nieuw = oud * (1 - percentage / 100);
            ...
            let toename = floor(random(10, 60));
            let nieuw = oud + toename;
            let percentage = ((nieuw - oud) / oud) * 100;
            
            text = "" + oud + " ⇒ " + nieuw + " \n =  ?%";   //  \u21D2 voor pijl
            answer = round(percentage * 100) / 100;  // 2 decimalen
        ...
        // Sla vraag EN antwoord op met UNIEKE ID
        questions.push({ 
            text: text,
            answer: answer,
            answerId: answerId,
        ...
        answers.push({
            value: answer,
            answerId: answerId,
            questionText: text  // Voor € of % bepaling
        });
    }

    // Shuffle alleen de antwoorden, niet de ID's!
    ...
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            ...
            blocks.push({
                ...
                text: q.text,
                answer: q.answer,
                answerId: q.answerId,  // UNIEKE ID
                triangleData: q.triangleData,
                isQuestion: true,
                isCorrect: null,
    ...
    for (let row = 3; row < 5; row++) {
        for (let col = 0; col < COLS; col++) {
            let a = answers[answerIndex];
            
            // Formatteer antwoord met 1 decimaal en komma
            let formattedAnswer = a.value.toFixed(1).replace('.', ',');
            
            // Bepaal of € of % symbool nodig is
            let answerText = "";
            if (a.questionText.includes("?%")) {
...
function checkAnswers() {
    ...
    for (let questionBlock of blocks) {
        ...
        // Zoek het antwoordblok met dezelfde answerId EN col EN row
        let answerBlock = blocks.find(b =>
            !b.isQuestion &&
            b.answerId === questionBlock.answerId &&
            b.col === questionBlock.col &&
            b.row === questionBlock.row  // 🔑 BELANGRIJKE TOEVOEGING!
        );

        if (!answerBlock) {
            // Geen antwoord op deze positie = fout
            questionBlock.isCorrect = false;
            continue;
        }

        // Vergelijk de numerieke waarden
        let questionAnswer = Number(questionBlock.answer);
        let givenAnswer = Number(answerBlock.answer);

        if (questionAnswer === givenAnswer) {
